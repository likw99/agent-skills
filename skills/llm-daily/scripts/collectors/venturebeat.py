...
class VentureBeatCollector(BaseCollector):
    """Collector for AI news from VentureBeat RSS feed"""

    def __init__(self, cache_dir: Optional[str] = None):
        self.rss_url = "https://venturebeat.com/category/ai/feed/"
        self.cache_dir = cache_dir or os.path.join(os.path.dirname(os.path.dirname(__file__)), "cache", "venturebeat")
        os.makedirs(self.cache_dir, exist_ok=True)

    def collect_data(self, days_back=7, max_articles=25, use_cache=True, force_refresh=False, **kwargs):
        cache_file = os.path.join(self.cache_dir, f"venturebeat_{days_back}d.json")
        ...
        if not force_refresh and use_cache and os.path.exists(cache_file):
            try:
                ...
                if (datetime.now() - cache_time).total_seconds() / 3600 < 12:
                    print("Using cached VentureBeat data")
                    return cached_data
            except Exception:
                pass
    ...
    def _fetch_rss(self, days_back):
        ...
        try:
            ...
            print(f"Error fetching VentureBeat RSS: {e}")
            return []
